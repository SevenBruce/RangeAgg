...
public class Agg {
	...
	public RepMessage2 getRepMessage(RepMessage2 repMessage) throws IOException {
		...
		return generateReportingMessage(sumUpReportingData());
	}
	...
	private Cij sumUpReportingData() throws IOException {
		...
		Element data[] = new Element[Params.NUMBER_OF_REPORTING_DATA_TYPE];
		for (int i = 0; i < Params.NUMBER_OF_REPORTING_DATA_TYPE; i++) {
			data[i] = pairing.getG1().newOneElement();
		}
		...
		while (itRep.hasNext()) {
			...
			for (int i = 0; i < Params.NUMBER_OF_REPORTING_DATA_TYPE; i++) {
				data[i].add(rep.getCij().getData()[i]);
			}
	...
	private Element PrepareLeft() {
		...
		Element temSum = pairing.getG1().newZeroElement();
		while (itRep.hasNext()) {
			temSum.add(itRep.next().getSij().getRij2G());
		}
		return temSum;
	}
